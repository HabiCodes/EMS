...
(function seedData() {
  ...
  for (var i = 0; i < 25; i++) {
    ...
    bookings.set(bid, {
      id: bid,
      type: type,
      user_email: 'user' + (i % 5 + 1) + '@test.com',
      user_username: 'User ' + (i % 5 + 1),
  ...
  for (var j = 0; j < 8; j++) {
    ...
    turfBookings.set(tid, {
      ...
      user_email: 'user' + (j % 5 + 1) + '@test.com',
      user_name: 'User ' + (j % 5 + 1),
...
function authUser(req) {
  ...
  if (!user) return null;
  return { email: user.email, username: user.username, verified: user.verified, phone: user.phone || null, id: user.id || user.email };
}
...
async function handleRegisterEnhanced(req, res) {
  ...
  users.set(email, { email, username, passwordHash: hashPassword(password, CUSTOMER_JWT_SECRET), verified: false, phone: '' });
...
async function handleListTurfsPublic(req, res) {
  ...
  if (q.city) items = items.filter(function(g) { return (g.city || '').toLowerCase() === q.city.toLowerCase(); });
  if (q.sport) items = items.filter(function(g) { return (g.sport || '').toLowerCase() === q.sport.toLowerCase(); });
  const result = paginate(items, q.page, q.pageSize);
  return jsonResponse(res, 200, { success: true, data: result.data, pagination: result.pagination });
}
...
async function handleUpdateProfile(req, res) {
  const user = authUser(req);
  if (!user) return jsonResponse(res, 401, { success: false, message: 'Not authenticated.' });
  const body = await readBody(req);
  const updates = {};
  if (typeof body.username === 'string') updates.username = body.username.slice(0, 50);
  if (typeof body.phone === 'string') updates.phone = body.phone.trim();
  if (!Object.keys(updates).length) return jsonResponse(res, 400, { success: false, message: 'Nothing to update.' });
  const existing = users.get(user.email);
  if (!existing) return jsonResponse(res, 404, { success: false, message: 'User not found.' });
  Object.assign(existing, updates);
  users.set(user.email, existing);
  return jsonResponse(res, 200, { success: true, data: {
    id: crypto.createHash('md5').update(existing.email).digest('hex').slice(0, 8),
...
// ── Customer/Admin: Users CRUD ────────────────────────────────────

async function handleListUsersPublic(req, res) {
  const user = authUser(req);
  const admin = authenticateAdmin(req);
  if (!user && !admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });

  const q = url.parse(req.url, true).query;
  let items = Array.from(users.values()).map(function(u) {
    return {
      id: crypto.createHash('md5').update(u.email).digest('hex').slice(0, 8),
      email: u.email,
      username: u.username,
...
async function handleGetUser(req, res, userId) {
  ...
  if (String(userId).indexOf('@') !== -1) {
    targetEmail = String(userId);
    targetUser = users.get(targetEmail);
  } else {
    for (const [email, u] of users.entries()) {
      const id = crypto.createHash('md5').update(email).digest('hex').slice(0, 8);
      if (id === String(userId)) {
        targetUser = u;
        targetEmail = email;
        break;
  ...
  if (!targetUser) return jsonResponse(res, 404, { success: false, error: 'User not found.' });

  if (!admin && user && user.email !== targetEmail) {
    return jsonResponse(res, 403, { success: false, error: 'Not authorized.' });
  }

  return jsonResponse(res, 200, {
    success: true,
    data: {
      id: crypto.createHash('md5').update(targetEmail).digest('hex').slice(0, 8),
      email: targetUser.email,
      username: targetUser.username,
      isVerified: targetUser.verified,
      isActive: true,
...
async function handleUpdateUser(req, res, userId) {
  ...
  if (admin) {
    ...
    if (typeof body.isVerified === 'boolean') updates.verified = body.isVerified;
  }

  if (!Object.keys(updates).length) return jsonResponse(res, 400, { success: false, error: 'Nothing to update.' });

  Object.assign(targetUser, updates);

  if (updates.email && updates.email !== targetEmail) {
    users.delete(targetEmail);
    users.set(updates.email, targetUser);
...
async function handleDeleteUser(req, res, userId) {
  ...
  users.delete(targetEmail);
  return jsonResponse(res, 200, { success: true, message: 'User deleted successfully.' });
}

// ── Admin Turf Admin (venues with status) ───────────────────────────

async function handleListAdminTurfVenues(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const q = url.parse(req.url, true).query;
  let items = Array.from(turfGrounds.values());
...
async function handleUpdateTurfVenueStatus(req, res, venueId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const body = await readBody(req);
  const venue = turfGrounds.get(venueId);
  if (!venue) return jsonResponse(res, 404, { success: false, error: 'Venue not found.' });
  if (body.status) venue.status = body.status;
  turfGrounds.set(venueId, venue);
  return jsonResponse(res, 200, { success: true, data: venue });
}

// ── Admin Movies ─────────────────────────────────────────────────────

async function handleAdminListMovies(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const q = url.parse(req.url, true).query;
  let items = Array.from(movies.values());
  if (q.genre) items = items.filter(function(m) { return m.genre === q.genre; });
  if (q.language) items = items.filter(function(m) { return m.language === q.language; });
  if (q.featured === 'true') items = items.filter(function(m) { return !!m.featured; });
  const result = paginate(items, q.page, q.pageSize);
...
async function handleAdminMovie(req, res, movieId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const movie = movies.get(movieId);
  if (!movie) return jsonResponse(res, 404, { success: false, error: 'Movie not found.' });
  if (req.method === 'GET') return jsonResponse(res, 200, { success: true, data: movie });
  if (req.method === 'DELETE') { movies.delete(movieId); return jsonResponse(res, 200, { success: true, message: 'Movie deleted.' }); }
  if (req.method === 'PUT' || req.method === 'PATCH') {
    const body = await readBody(req);
    Object.assign(movie, body);
    movies.set(movieId, movie);
    return jsonResponse(res, 200, { success: true, data: movie });
  }
  return jsonResponse(res, 405, { success: false, error: 'Method not allowed.' });
}
...
async function handleAdminCreateMovie(req, res) {
  ...
  const body = await readBody(req);
  const id = genId();
  const movie = Object.assign({ id: id, is_active: true }, body);
  movies.set(id, movie);
  return jsonResponse(res, 201, { success: true, data: movie });
}

async function handleAdminPublishMovie(req, res, movieId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const movie = movies.get(movieId);
  if (!movie) return jsonResponse(res, 404, { success: false, error: 'Movie not found.' });
  ...
  movies.set(movieId, movie);
  return jsonResponse(res, 200, { success: true, data: movie, message: 'Movie published.' });
}

async function handleAdminArchiveMovie(req, res, movieId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const movie = movies.get(movieId);
  if (!movie) return jsonResponse(res, 404, { success: false, error: 'Movie not found.' });
  ...
  movies.set(movieId, movie);
  return jsonResponse(res, 200, { success: true, data: movie, message: 'Movie archived.' });
}

// ── Admin Cinemas ────────────────────────────────────────────────────

async function handleAdminListCinemas(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const q = url.parse(req.url, true).query;
  let items = Array.from(cinemas.values());
...
async function handleAdminCinema(req, res, cinemaId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const cinema = cinemas.get(cinemaId);
  if (!cinema) return jsonResponse(res, 404, { success: false, error: 'Cinema not found.' });
  if (req.method === 'GET') return jsonResponse(res, 200, { success: true, data: cinema });
  if (req.method === 'DELETE') { cinemas.delete(cinemaId); return jsonResponse(res, 200, { success: true, message: 'Cinema deleted.' }); }
  if (req.method === 'PUT' || req.method === 'PATCH') {
    const body = await readBody(req);
    Object.assign(cinema, body);
    cinemas.set(cinemaId, cinema);
    return jsonResponse(res, 200, { success: true, data: cinema });
  }
  return jsonResponse(res, 405, { success: false, error: 'Method not allowed.' });
}

async function handleAdminCreateCinema(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const body = await readBody(req);
  const id = genId();
  const cinema = Object.assign({ id: id, is_active: true }, body);
  cinemas.set(id, cinema);
  return jsonResponse(res, 201, { success: true, data: cinema });
}

async function handleAdminToggleCinema(req, res, cinemaId, action) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const cinema = cinemas.get(cinemaId);
  if (!cinema) return jsonResponse(res, 404, { success: false, error: 'Cinema not found.' });
  if (action === 'deactivate') cinema.is_active = false;
  else cinema.is_active = !cinema.is_active;
  cinemas.set(cinemaId, cinema);
  return jsonResponse(res, 200, { success: true, data: cinema, message: 'Cinema ' + (cinema.is_active ? 'activated' : 'deactivated') });
}
...
async function handleCreateOrganization(req, res) {
  ...
  return jsonResponse(res, 201, { success: true, data: org });
}

async function handleUpdateOrganization(req, res, orgId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const body = await readBody(req);
  const org = organizations.get(orgId);
  ...
  organizations.set(orgId, org);
  return jsonResponse(res, 200, { success: true, data: org });
}

// ── Admin recent-tickets ──────────────────────────────────────────────

async function handleAdminRecentTickets(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const q = url.parse(req.url, true).query;
  const limit = parseInt(q.limit || '10', 10);
  let items = Array.from(bookings.values()).sort(function(a, b) {
    return new Date(b.created_at) - new Date(a.created_at);
  }).slice(0, limit);
  return jsonResponse(res, 200, { success: true, data: items.map(function(b) {
    return {
      id: b.id,
      event_title: b.event_title,
      user_email: b.user_email,
      user_name: b.user_username,
      status: b.status,
...
async function handleAdminListShowtimes(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const q = url.parse(req.url, true).query;
  const allCids = Array.from(cinemas.keys());
  const allMids = Array.from(movies.keys());
  let items = [];
  for (var i = 0; i < 30; i++) {
    var cin = cinemas.get(allCids[i % allCids.length]);
    var mov = movies.get(allMids[i % allMids.length]);
    items.push({
      id: 'showtime-' + i,
      cinema_id: cin ? cin.id : allCids[i % allCids.length],
      cinema_name: cin ? cin.name : 'Cinema',
      cinema_city: cin ? (cin.city || '') : '',
      movie_id: mov ? mov.id : allMids[i % allMids.length],
      movie_title: mov ? mov.title : 'Movie',
      screen: 'Screen ' + ((i % 8) + 1),
      show_date: '2026-0' + (4 + (i % 6)) + '-' + String(10 + (i % 20)).padStart(2, '0'),
      show_time: String((9 + (i % 12)).toString().padStart(2, '0')) + ':' + (i % 4 === 0 ? '00' : '30'),
...
async function handleAdminShowtime(req, res, showtimeId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  if (req.method === 'DELETE') return jsonResponse(res, 200, { success: true, message: 'Showtime deleted.' });
  return jsonResponse(res, 200, { success: true, data: { id: showtimeId } });
}

async function handleAdminCreateShowtime(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const body = await readBody(req);
  const id = 'showtime-' + Date.now();
  const showtime = Object.assign({ id: id }, body);
  return jsonResponse(res, 201, { success: true, data: showtime });
}

async function handleAdminMovieScreens(req, res, cinemaId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const items = [];
  for (var i = 1; i <= 4; i++) {
    items.push({
      id: 'screen-' + cinemaId + '-' + i,
      cinema_id: cinemaId,
      screen_number: i,
      screen_name: 'Screen ' + i,
      seat_capacity: 120 + i * 10,
      screen_type: ['2D', '3D', 'IMAX'][i % 3],
      is_active: true,
    });
  }
  return jsonResponse(res, 200, { success: true, data: items });
...
const banners = new Map([
  ['banner-1', { id: 'banner-1', title: 'Welcome Event', type: 'event', image_url: '', target_id: 'evt-1', is_active: true, position: 'top', created_at: new Date().toISOString() }],
  ['banner-2', { id: 'banner-2', title: 'Weekend Deals', type: 'promotion', image_url: '', target_id: null, is_active: false, position: 'sidebar', created_at: new Date().toISOString() }],
]);

async function handleListBanners(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const items = Array.from(banners.values());
  const result = paginate(items, url.parse(req.url, true).query.page, url.parse(req.url, true).query.pageSize);
  return jsonResponse(res, 200, { success: true, data: result.data, pagination: result.pagination });
}
async function handleGetBanner(req, res, bannerId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const b = banners.get(bannerId);
  if (!b) return jsonResponse(res, 404, { success: false, error: 'Banner not found.' });
  return jsonResponse(res, 200, { success: true, data: b });
}
async function handleCreateBanner(req, res) {
  ...
  const body = await readBody(req);
  const id = genId();
  const b = Object.assign({ id, is_active: true, created_at: new Date().toISOString() }, body);
  banners.set(id, b);
  return jsonResponse(res, 201, { success: true, data: b });
}
async function handleUpdateBanner(req, res, bannerId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const body = await readBody(req);
  const b = banners.get(bannerId);
  ...
  banners.set(bannerId, b);
  return jsonResponse(res, 200, { success: true, data: b });
}
async function handleDeleteBanner(req, res, bannerId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const b = banners.get(bannerId);
  if (!b) return jsonResponse(res, 404, { success: false, error: 'Banner not found.' });
  banners.delete(bannerId);
  return jsonResponse(res, 200, { success: true, message: 'Banner deleted.' });
}
async function handleBannerAction(req, res, bannerId, action) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const b = banners.get(bannerId);
  if (!b) return jsonResponse(res, 404, { success: false, error: 'Banner not found.' });
  b.is_active = action === 'activate';
  banners.set(bannerId, b);
  return jsonResponse(res, 200, { success: true, data: b });
}
...
const mediaLibrary = new Map([
  ['media-1', { id: 'media-1', type: 'image', url: 'https://picsum.photos/400/200?1', filename: 'event1.jpg', size: 50000, mime_type: 'image/jpeg', uploaded_by: 'admin', created_at: new Date().toISOString() }],
]);

async function handleListMedia(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const items = Array.from(mediaLibrary.values());
  const result = paginate(items, url.parse(req.url, true).query.page, url.parse(req.url, true).query.pageSize);
  return jsonResponse(res, 200, { success: true, data: result.data, pagination: result.pagination });
}
async function handleMediaDetail(req, res, mediaId) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const m = mediaLibrary.get(mediaId);
  if (!m) return jsonResponse(res, 404, { success: false, error: 'Media not found.' });
  if (req.method === 'DELETE') { mediaLibrary.delete(mediaId); return jsonResponse(res, 200, { success: true, message: 'Media deleted.' }); }
  if (req.method === 'PUT' || req.method === 'PATCH') {
    const body = await readBody(req);
    Object.assign(m, body);
    mediaLibrary.set(mediaId, m);
    return jsonResponse(res, 200, { success: true, data: m });
  }
  return jsonResponse(res, 200, { success: true, data: m });
}
async function handleCreateMedia(req, res) {
  const admin = authenticateAdmin(req);
  if (!admin) return jsonResponse(res, 401, { success: false, error: 'Not authenticated.' });
  const body = await readBody(req);
  const id = genId();
  const m = Object.assign({ id: id, uploaded_by: admin.email, created_at: new Date().toISOString() }, body);
  mediaLibrary.set(id, m);
  return jsonResponse(res, 201, { success: true, data: m });
}
...
const routes = {
  ...
  'PATCH:/api/v1/auth/me': handleUpdateProfile,
  ...
  'GET:/api/v1/users': handleListUsersPublic,
  'GET:/api/v1/users/:id': handleGetUser,
  'PATCH:/api/v1/users/:id': handleUpdateUser,
  ...
  // Admin banners
  'GET:/api/v1/admin/banners': handleListBanners,
  'GET:/api/v1/admin/banners/:id': handleGetBanner,
  'POST:/api/v1/admin/banners': handleCreateBanner,
  'PUT:/api/v1/admin/banners/:id': handleUpdateBanner,
  'DELETE:/api/v1/admin/banners/:id': handleDeleteBanner,
  'POST:/api/v1/admin/banners/:id/activate': handleBannerAction,
  'POST:/api/v1/admin/banners/:id/deactivate': handleBannerAction,
  // Admin media
  'GET:/api/v1/admin/media': handleListMedia,
  'GET:/api/v1/admin/media/:id': handleMediaDetail,
  'POST:/api/v1/admin/media': handleCreateMedia,
  'PUT:/api/v1/admin/media/:id': handleMediaDetail,
  'PATCH:/api/v1/admin/media/:id': handleMediaDetail,
  'DELETE:/api/v1/admin/media/:id': handleMediaDetail,
  ...
  'GET:/api/v1/admin/movies/cinemas/:id': handleAdminCinema,
  'PUT:/api/v1/admin/movies/cinemas/:id': handleAdminCinema,
  'DELETE:/api/v1/admin/movies/cinemas/:id': handleAdminCinema,
  'POST:/api/v1/admin/movies/cinemas/:id/activate': handleAdminToggleCinema,
  'POST:/api/v1/admin/movies/cinemas/:id/deactivate': handleAdminToggleCinema,
  'POST:/api/v1/admin/movies/cinemas': handleAdminCreateCinema,
  'GET:/api/v1/admin/movies/cinemas/:id/screens': handleAdminMovieScreens,

  // Admin movies
  'GET:/api/v1/admin/movies': handleAdminListMovies,
  'GET:/api/v1/admin/movies/:id': handleAdminMovie,
  'PUT:/api/v1/admin/movies/:id': handleAdminMovie,
  'DELETE:/api/v1/admin/movies/:id': handleAdminMovie,
  'POST:/api/v1/admin/movies': handleAdminCreateMovie,
  'POST:/api/v1/admin/movies/:id/publish': handleAdminPublishMovie,
  'POST:/api/v1/admin/movies/:id/archive': handleAdminArchiveMovie,
  // Movies alias routes (frontend uses /admin/movies/movies)
  'GET:/api/v1/admin/movies/movies': handleAdminListMovies,
  'GET:/api/v1/admin/movies/movies/:id': handleAdminMovie,
  'PUT:/api/v1/admin/movies/movies/:id': handleAdminMovie,
  'DELETE:/api/v1/admin/movies/movies/:id': handleAdminMovie,
  'POST:/api/v1/admin/movies/movies/:id/publish': handleAdminPublishMovie,
  'POST:/api/v1/admin/movies/movies/:id/archive': handleAdminArchiveMovie,
  'POST:/api/v1/admin/movies/movies': handleAdminCreateMovie,

  // Admin showtimes
  'GET:/api/v1/admin/showtimes': handleAdminListShowtimes,
  'GET:/api/v1/admin/showtimes/:id': handleAdminShowtime,
  'DELETE:/api/v1/admin/showtimes/:id': handleAdminShowtime,
  'POST:/api/v1/admin/showtimes': handleAdminCreateShowtime,

  // Admin organizations
  'GET:/api/v1/admin/organizations': handleListOrganizations,
  'GET:/api/v1/admin/organizations/:id': handleListOrganizations,
  'PUT:/api/v1/admin/organizations/:id': handleUpdateOrganization,
  'POST:/api/v1/admin/organizations': handleCreateOrganization,
  'POST:/api/v1/admin/organizations/:id/deactivate': handleOrgAction,
  'POST:/api/v1/admin/organizations/:id/reactivate': handleOrgAction,

  // Admin recent tickets
  'GET:/api/v1/admin/recent-tickets': handleAdminRecentTickets,

  // Admin turf venues with status
  'GET:/api/v1/admin/turf/grounds': handleListAllGrounds,
  'POST:/api/v1/admin/turf/grounds/:id/activate': handleUpdateTurfVenueStatus,
  'POST:/api/v1/admin/turf/grounds/:id/deactivate': handleUpdateTurfVenueStatus,
  // Turf admin alias routes (frontend uses /turf/admin/*)
  'GET:/api/v1/turf/admin/venues': handleListAllGrounds,
  'GET:/api/v1/turf/admin/venues/:id': handleListAllGrounds,
  'PATCH:/api/v1/turf/admin/venues/:id/status': handleUpdateTurfVenueStatus,
  'GET:/api/v1/turf/admin/bookings': handleListTurfBookings,
  'GET:/api/v1/turf/admin/bookings/:id': handleListTurfBookings,
  'GET:/api/v1/turf/admin/venues/:id/reviews': handleListTurfBookings,

  // Admin refunds
  'GET:/api/v1/admin/refunds': handleListRefunds,
  'GET:/api/v1/admin/refunds/:id': handleListRefunds,
  'POST:/api/v1/admin/refunds': handleListRefunds,
  ...
  // Admin managers
  'GET:/api/v1/admin/managers': handleListManagers,
  'GET:/api/v1/admin/managers/:id': handleListManagers,
  'PUT:/api/v1/admin/managers/:id': handleListManagers,
  'POST:/api/v1/admin/managers': handleCreateManager,
  'POST:/api/v1/admin/managers/:id/deactivate': handleManagerAction,
  'POST:/api/v1/admin/managers/:id/reactivate': handleManagerAction,
...
const server = http.createServer(async function(req, res) {
  ...
  try {
    ...
    if (pathname.match(/^\/api\/v1\/admin\/organizations\/\d+$/)) {
      var m7 = pathname.match(/\/organizations\/(\d+)$/);
      if (m7) return await handleListOrganizations(req, res, m7[1]);
    }
    if (pathname.match(/^\/api\/v1\/admin\/organizations$/)) {
      if (method === 'POST') return await handleCreateOrganization(req, res);
    }
    if (pathname.match(/^\/api\/v1\/admin\/movies\/\d+$/)) {
      var m8 = pathname.match(/\/movies\/(\d+)$/);
      if (m8) return await handleAdminMovie(req, res, m8[1]);
    }
    if (pathname.match(/^\/api\/v1\/admin\/movies$/)) {
      if (method === 'POST') return await handleAdminCreateMovie(req, res);
    }
    if (pathname.match(/^\/api\/v1\/admin\/movies\/\d+\/(publish|archive)$/)) {
      var m9 = pathname.match(/\/movies\/(\d+)\/(\w+)$/);
      if (m9) {
        if (m9[2] === 'publish') return await handleAdminPublishMovie(req, res, m9[1]);
        if (m9[2] === 'archive') return await handleAdminArchiveMovie(req, res, m9[1]);
      }
    }
    if (pathname.match(/^\/api\/v1\/admin\/movies\/cinemas\/\d+$/)) {
      var m10 = pathname.match(/\/cinemas\/(\d+)$/);
      if (m10) return await handleAdminCinema(req, res, m10[1]);
    }
    if (pathname.match(/^\/api\/v1\/admin\/movies\/cinemas\/\d+\/(activate|deactivate)$/)) {
      var m11 = pathname.match(/\/cinemas\/(\d+)\/(\w+)$/);
      if (m11) return await handleAdminToggleCinema(req, res, m11[1], m11[2]);
    }
    if (pathname.match(/^\/api\/v1\/admin\/movies\/cinemas$/)) {
      if (method === 'POST') return await handleAdminCreateCinema(req, res);
    }
    if (pathname.match(/^\/api\/v1\/admin\/movies\/cinemas\/\d+\/screens$/)) {
      var m12 = pathname.match(/\/cinemas\/(\d+)\/screens$/);
      if (m12) return await handleAdminMovieScreens(req, res, m12[1]);
    }
    if (pathname.match(/^\/api\/v1\/admin\/showtimes\/\d+$/)) {
      var m13 = pathname.match(/\/showtimes\/(\d+)$/);
      if (m13) return await handleAdminShowtime(req, res, m13[1]);
    }
    if (pathname.match(/^\/api\/v1\/admin\/showtimes$/)) {
      if (method === 'POST') return await handleAdminCreateShowtime(req, res);
    }
    if (pathname.match(/^\/api\/v1\/admin\/turf\/grounds\/\d+\/(activate|deactivate)$/)) {
      var m14 = pathname.match(/\/grounds\/(\d+)\/(\w+)$/);
      if (m14) return await handleUpdateTurfVenueStatus(req, res, m14[1]);
    }
    if (pathname.match(/^\/api\/v1\/admin\/managers\/\d+$/)) {
      var m15 = pathname.match(/\/managers\/(\d+)$/);
      if (m15) return await handleListManagers(req, res, m15[1]);
    }
    if (pathname.match(/^\/api\/v1\/admin\/refunds\/\d+$/)) {
      var m16 = pathname.match(/\/refunds\/(\d+)$/);
      if (m16) return await handleListRefunds(req, res, m16[1]);
    }

    if (pathname.match(/^\/api\/v1\/users\/[A-Za-z0-9_-]+$/)) {
      var m7 = pathname.match(/\/users\/([A-Za-z0-9_-]+)$/);
      if (m7) return await matched.handler(req, res, m7[1]);
    }
