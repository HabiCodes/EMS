...
var ShowtimesListPage = (function () {
  ...
  function renderHeader() {
    ...
    info.appendChild(DOM.el('p', { className: 'page-header__subtitle', textContent: 'Cinema showtime schedules' }));
  ...
  function renderFilters() {
    ...
    var searchInput = DOM.el('input', { className: 'form-input', type: 'text', placeholder: 'Search showtimes…', id: 'showtimeSearch', style: 'min-width:240px;' });
  ...
  function renderTable() {
    ...
    var container = DOM.el('div', { className: 'table-container', id: 'showtimesTableContainer' });
    container.appendChild(Components.spinner('Loading showtimes…'));
    body.appendChild(container);
    var pagination = DOM.el('div', { className: 'pagination', id: 'showtimesPagination' });
  ...
  async function loadData() {
    var container = document.getElementById('showtimesTableContainer');
    var paginationEl = document.getElementById('showtimesPagination');
    if (!container) return;
    container.innerHTML = '';
    container.appendChild(Components.spinner('Loading showtimes…'));
    ...
    try {
      ...
      var response = await AdminShowtimesAPI.list(params);
  ...
  function renderRows(items, container) {
    ...
    ['ID', 'Movie', 'Cinema', 'Screen', 'Date', 'Time', 'Price', 'Seats', ''].forEach(function (h) {
    ...
    items.forEach(function (s) {
      var row = DOM.el('tr');
      row.appendChild(DOM.el('td', { textContent: s.id || '—' }));
      row.appendChild(DOM.el('td', { textContent: FormatUtil.escHtml(s.movie_title || s.movieTitle || '—') }));
      row.appendChild(DOM.el('td', { textContent: FormatUtil.escHtml(s.cinema_name || s.cinemaName || '—') }));
      row.appendChild(DOM.el('td', { textContent: FormatUtil.escHtml(s.screen || '—') }));
      row.appendChild(DOM.el('td', { textContent: s.show_date || s.showDate || '—' }));
      row.appendChild(DOM.el('td', { textContent: s.show_time || s.showTime || '—' }));
      row.appendChild(DOM.el('td', { textContent: s.price != null ? FormatUtil.formatCount(s.price) : '—' }));
      row.appendChild(DOM.el('td', { textContent: (s.available_seats != null ? s.available_seats : (s.remaining_seats != null ? s.remaining_seats : '—')) }));
      var cell = DOM.el('td', { style: 'white-space:nowrap;' });
      var editBtn = DOM.el('button', { className: 'btn btn--ghost btn--sm', textContent: 'Edit' });
      editBtn.addEventListener('click', function () { Components.showToast('Showtime editing coming soon', 'info'); });
