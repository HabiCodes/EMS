/**
 * Organizations List page module — Super Admin.
 *
 * Lists all organizations with search, create, update, deactivate/reactivate.
 */
...
var OrganizationsListPage = (function () {
  ...
  function renderHeader() {
    ...
    info.appendChild(DOM.el('p', { className: 'page-header__subtitle', textContent: 'Manage all event organizations on the platform' }));
    header.appendChild(info);
    var actions = DOM.el('div', { className: 'page-header__actions' });
    var createBtn = DOM.el('button', { className: 'btn btn--primary', textContent: '+ New Organization' });
  ...
  function renderFilters() {
    ...
    var searchInput = DOM.el('input', { className: 'form-input', type: 'text', placeholder: 'Search organizations…', id: 'orgSearch', style: 'min-width:240px;' });
  ...
  function renderTable() {
    ...
    var container = DOM.el('div', { className: 'table-container', id: 'orgTableContainer' });
    container.appendChild(Components.spinner('Loading organizations…'));
    body.appendChild(container);
    var pagination = DOM.el('div', { className: 'pagination', id: 'orgPagination' });
  ...
  async function loadData() {
    var container = document.getElementById('orgTableContainer');
    var paginationEl = document.getElementById('orgPagination');
    if (!container) return;
    container.innerHTML = '';
    ...
    try {
      ...
      var response = await AdminOrganizationsAPI.list(params);
  ...
  function renderRows(items, container) {
    DOM.empty(container);
    if (items.length === 0) {
      container.appendChild(Components.emptyState('🏢', 'No organizations found', 'Create the first organization to get started.'));
      return;
    }
    var table = DOM.el('table', { className: 'data-table' });
    var thead = DOM.el('thead');
    var headRow = DOM.el('tr');
    ['ID', 'Name', 'Contact Email', 'Contact Phone', 'Type', 'Status', 'Created', 'Actions'].forEach(function (h) {
    ...
    items.forEach(function (org) {
      ...
      row.appendChild(DOM.el('td', { textContent: FormatUtil.escHtml(org.email || org.contact_email || '—') }));
      row.appendChild(DOM.el('td', { textContent: org.phone || org.contact_phone || '—' }));
      row.appendChild(DOM.el('td', { textContent: org.type || '—' }));
      row.appendChild(DOM.el('td', { textContent: Components.badge(org.is_active !== false ? 'Active' : 'Inactive', org.is_active !== false ? 'green' : 'red') }));
      ...
      editBtn.addEventListener('click', function () { openEditModal(org.id); });
      ...
      if (org.is_active !== false) {
        ...
        deactBtn.addEventListener('click', function () {
          Components.confirmDialog('Deactivate Organization', 'Deactivate "' + FormatUtil.escHtml(org.name) + '"?', function () {
            AdminOrganizationsAPI.deactivate(org.id).then(function () {
              Components.showToast('Organization deactivated', 'success');
              loadData();
        ...
        actBtn.addEventListener('click', function () {
          AdminOrganizationsAPI.reactivate(org.id).then(function () {
            Components.showToast('Organization reactivated', 'success');
            loadData();
          }).catch(function (err) { Components.showToast(err.message, 'error'); });
        });
  ...
  function openEditModal(orgId) {
    var isEdit = !!orgId;
    var title = isEdit ? 'Edit Organization' : 'New Organization';
    var content = DOM.el('form', { id: 'orgForm' });
    var fields = [
      { name: 'name', label: 'Organization Name', type: 'text', required: true },
      { name: 'email', label: 'Contact Email', type: 'email' },
      { name: 'phone', label: 'Contact Phone', type: 'text' },
      { name: 'type', label: 'Type', type: 'text', placeholder: 'e.g. event, cinema, turf' },
    ...
    content.addEventListener('submit', function (e) {
      ...
      if (isEdit) {
        AdminOrganizationsAPI.update(orgId, data).then(function () {
          Components.showToast('Organization updated', 'success');
          AdminShell.closeModal();
          loadData();
        }).catch(function (err) { Components.showToast(err.message, 'error'); });
      } else {
        AdminOrganizationsAPI.list({ page: 1, pageSize: 1 }).then(function () {
          // Use the create endpoint via the adminOrganizerController
          AdminAPI.post('/admin/organizations', data).then(function () {
            Components.showToast('Organization created', 'success');
            AdminShell.closeModal();
            loadData();
          }).catch(function (err) { Components.showToast(err.message, 'error'); });
        });
    ...
    if (isEdit) {
      AdminOrganizationsAPI.get(orgId).then(function (resp) {
        var data = resp.data || resp;
        var fields2 = content.querySelectorAll('input');
        fields2.forEach(function (input) {
